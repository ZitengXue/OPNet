    ...
    with inference_context(model), torch.no_grad():
        with EventStorage() as storage:
            for idx, inputs in enumerate(data_loader):
                ...
                # outputs1 = model(inputs, do_postprocess=False)
                # # #
                ...
                # # indice_list1=list(range(len(inputs[0]['inference_instances'].gt_classes)))
                # # gt_img = vis_image(inputs[0], inputs[0], mode="gt", colors=co, ind_lst=indice_list1)
                # # # vis.images(gt_img, win_name='gt')
                # # im = Image.fromarray(gt_img.transpose(1, 2, 0))
                # # im.save("/data0/ziteng/visualizer/D2SA/gt/box/ours_{}_gt.jpg".format(idx))
                
                # outputs_resize = detector_postprocess(outputs1[0], inputs[0]['image'].size(1), inputs[0]['image'].size(2))
                # # import ipdb
                # # ipdb.set_trace()
                # #print(outputs_resize.pred_amodal_masks.size(0))
                # indice_list=list(range(0,outputs_resize.pred_amodal_masks.size(0)))
                
                # #print(indice_list)
                # pred_img = vis_image(inputs[0], outputs_resize, mode="pred", colors=None, ind_lst=indice_list)
                # im = Image.fromarray(pred_img.transpose(1, 2, 0))
                # im.save("/data0/ziteng/visualizer/D2SA/v1/box/ours_{}_gt.jpg".format(idx))
...
def vis_image(inputs, instance, mode="gt", colors=None, ind_lst=None):
    ...
    if mode == "gt":
        v_output = v_output.overlay_instances(boxes=instance["inference_instances"].gt_boxes[ind_lst].tensor.cpu(), assigned_colors=colors)
    else:
        #v_output = v_output.overlay_instances(masks=instance.pred_amodal_masks[ind_lst].cpu(), assigned_colors=colors)
