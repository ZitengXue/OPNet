...
def fast_rcnn_inference(boxes, scores, image_shapes, score_thresh, nms_thresh, topk_per_image,
                        mask_pooler=None, mask_head=None, recon_net=None, features=None, recon_alpha=None, recls=None,soft_nms_enabled=False,
    ...
    if mask_pooler is not None and mask_head is not None:
        ...
        result_per_image = [
            fast_rcnn_inference_single_image_recon_recls(
                ...
                topk_per_image, features, mask_pooler, mask_head, recon_net, recon_alpha, recls,soft_nms_enabled,soft_nms_method,
                soft_nms_sigma,soft_nms_prune
            )
...
def fast_rcnn_inference_single_image(
    boxes, scores, image_shape, score_thresh, nms_thresh, topk_per_image,soft_nms_method="linear",
                soft_nms_sigma=0.5,soft_nms_prune=0.001
):
    """
...
def fast_rcnn_inference_single_image_recon_recls(
    boxes, scores, image_shape, score_thresh, nms_thresh, topk_per_image, features, mask_pooler, mask_head,
        recon_net=None, alpha=2, recls=None,soft_nms_enabled=False,soft_nms_method="linear",
                soft_nms_sigma=0.5,soft_nms_prune=0.001
):
    """
    Single-image inference. Return bounding-box detection results by thresholding
...
class FastRCNNOutputs(object):
    ...
    def _log_accuracy(self):
        ...
        pred_classes = self.pred_class_logits.argmax(dim=1)   #1024 [0,60]
        bg_class_ind = self.pred_class_logits.shape[1] - 1   #60

        fg_inds = (self.gt_classes >= 0) & (self.gt_classes < bg_class_ind)   #1024 bool
        num_fg = torch.nonzero(fg_inds,as_tuple=False).numel()   #9
        fg_gt_classes = self.gt_classes[fg_inds]            # 九个正样本的gt
        fg_pred_classes = pred_classes[fg_inds]             #九个正样本的pred class

        num_false_negative = torch.nonzero((fg_pred_classes == bg_class_ind),as_tuple=False).numel()    #预测为背景的个数
        num_accurate = torch.nonzero((pred_classes == self.gt_classes),as_tuple=False).numel()          #总样本中判断对错的个数
        fg_num_accurate = torch.nonzero((fg_pred_classes == fg_gt_classes),as_tuple=False).numel()      # positive样本中判断对错的个数
    ...
    def smooth_l1_loss(self):
        ...
        #这段代码中为什么box regression只计算分类正确的proposal的loss_box_reg，为什么最后除以GT框的数量而不是分类正确的proposal的数量
        #解释如下：
    ...
    def inference(self, score_thresh, nms_thresh, topk_per_image, mask_pooler=None, mask_head=None,
        ...
        return fast_rcnn_inference(
            ...
            mask_head=mask_head, features=features, recon_net=recon_net, recon_alpha=recon_alpha, recls=recls,soft_nms_enabled=soft_nms_enabled,
            soft_nms_method=soft_nms_method,
            soft_nms_sigma=soft_nms_sigma,
            soft_nms_prune=soft_nms_prune
        )
