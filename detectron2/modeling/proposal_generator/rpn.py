...
class RPN(nn.Module):
    ...
    def forward(self, images, features, gt_instances=None):
        ...
        #RPN_Head 前向传播一个3*3 1填充 步长1的卷积然后接一个in_chanels * 3 卷积计算三种尺度框的分数 
        #一个in_chanels * (3*4) box回归参数 
        #pred_objectness_logits[0]的维度是[2, 3, 192, 232]，就是在p2特征图上生成的3*192*232=133632个anchor的是否含有物体的得分以及他们的box偏移回归
        anchors = self.anchor_generator(features)
        #anchors跟上面的logits得分维度对应  anchors[0][0] p2 特征图上的133632个boxes
        #后面RPNOutputs应该找出1000个把box_deltas应用在box上就是真正的proposal？
...
# num_instances 1000    image_height 800  image_width  1067
# boxes 一共1000个 以及他们的分数objctness_logits tensor