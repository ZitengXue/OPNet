...
def find_top_rpn_proposals(
    ...
    Args:
        ...
        nms_thresh (float): IoU threshold to use for NMS   0.7
        pre_nms_topk (int): number of top k scoring proposals to keep before applying NMS.
            When RPN is run on multiple feature maps (as in FPN) this number is per
            feature map.   2000
        post_nms_topk (int): number of top k scoring proposals to keep after applying NMS.
            When RPN is run on multiple feature maps (as in FPN) this number is total,
            over all feature maps.  1000
    ...
    for level_id, proposals_i, logits_i in zip(
        ...
        Hi_Wi_A = logits_i.shape[1]   #每个特征图anchor个数
        num_proposals_i = min(pre_nms_topk, Hi_Wi_A) #2000

        # sort is faster than topk (https://github.com/pytorch/pytorch/issues/22812)
        # topk_scores_i, topk_idx = logits_i.topk(num_proposals_i, dim=1)
        logits_i, idx = logits_i.sort(descending=True, dim=1)   #将logits从高往低排并记录排序后的下标
        topk_scores_i = logits_i[batch_idx, :num_proposals_i]  #(2,2000)的分数
        topk_idx = idx[batch_idx, :num_proposals_i]       #(2,2000)的索引

        # each is N x topk
        topk_proposals_i = proposals_i[batch_idx[:, None], topk_idx]  # N x topk x 4   根据索引找到proposals

        topk_proposals.append(topk_proposals_i)
        topk_scores.append(topk_scores_i)
        level_ids.append(torch.full((num_proposals_i,), level_id, dtype=torch.int64, device=device))
    #每个尺度图都提出来2000，但是最后一个一般提不满
...
class RPNOutputs(object):
    ...
    def _get_ground_truth(self):
        ...
        for image_size_i, anchors_i, gt_boxes_i in zip(self.image_sizes, anchors, self.gt_boxes):
            ...
            #计算实例gt的框和生成的所有chanors对应的iou     match_quality_matrix大小为(实例的个数，anchors的个数)
            matched_idxs, gt_objectness_logits_i = retry_if_cuda_oom(self.anchor_matcher)(
                match_quality_matrix
            )
            #matched_idxs每个proposal anchor匹配的gt框，gt_objectness_logits表示这个框是否存在物体 
            ...
            #如果匹配到了框 就记录box之间的转换，否则deltas为0
            if len(gt_boxes_i) == 0:
                ...
                #记录anchors到gt的回归参数
            #这两个数组记录所有的bs的logits和anchor deltas
    ...
    def losses(self):
        ...
        """
#从一个混合了正负样本标签的标签向量中随机抽取一定数量的正负样本索引
        def resample(label):
            """
        ...
        #返回bs个logits得分  是经过thresh阈值处理后的，和box转换的四个参数
        #logits要么为0要么为1 为1记录box回归参数， 为0 回归参数全0
        ...
        #num_anchors_per_map是每个尺度特征图上的anchor数量[112896,28224,7056,1764,462]
        #num_anchors_per_image是每张图片生成的anchor数量就是上面的加一起是150402
